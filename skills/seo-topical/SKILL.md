---
name: seo-topical
description: Topical authority e arquitetura de conteúdo: mapas de tópicos, clusters, entidades e cobertura de intenção (escola Koray + Dixon Jones). Use quando o usuário pedir "montar topical map", "arquitetura de conteúdo", "mapa de tópicos", "quero dominar esse nicho", "estrutura de clusters", "pillar e hub", "cobertura semântica", ou "quais conteúdos criar".
user-invokable: true
...
A escola de referência é **Koray Tuğberk Gübür** (topical authority por cobertura) combinada com **Dixon Jones** (entidades como nós do grafo). A questão que você responde não é "sobre o que escrever" — é "qual entidade dominar, de que ângulos, em que profundidade e em que sequência".
...
- **Pillar (macro-silo):** responde à pergunta fundamental sobre aquele ângulo da entidade. Não publica antes de ter 70%+ dos hubs filhos mapeados. Publicar um pillar vazio de filhos é publicar um nó sem contexto — o Google não encontra o que confirme a cobertura declarada.
- **Hub (sub-silo):** aprofunda um aspecto específico do pillar. Não publica sem 5+ clusters mapeados. Sem clusters filhos, o hub não demonstra profundidade — apenas largura superficial.
  ...
  "macro_silos": [
    {
      ...
      "search_intent": "informacional|comercial|transacional",
      "sub_silos": [
        {
          ...
          "clusters": [
            {
              ...
              "information_gain_elements": [
                ...
                "Elemento de IG 2 (ex: dado primário de pesquisa com N=?)",
...
**Fluxo obrigatório:**
- Cluster → Hub pai (âncora = nome da entidade do hub)
- Cluster ↔ Clusters irmãos no mesmo hub (contextual vicinity — 1 a 2 links por cluster, âncoras variadas)
- Hub → Pillar pai (âncora = nome da entidade do macro-silo)
- Pillar → Hubs filhos (todos, progressivamente à medida que publicam)

**Regras de âncora:**
- Nunca a mesma âncora duas vezes na mesma página
- Nunca âncoras genéricas ("clique aqui", "saiba mais", "leia também")
- Âncoras são fragmentos semânticos da entidade destino: variam entre o nome da entidade, um atributo dela, ou uma relação dela com a página de origem
...
## 8. Modo Execução: priorização por oportunidade real
...
## 9. Modo Framework: o que coletar antes de priorizar
...
5. **Próximo passo concreto:** a próxima URL a produzir, com link para a sub-skill `seo-on-page` para gerar o content brief
...
- Não define estratégia de link building — isso é `seo-auditoria` (seção de autoridade)
